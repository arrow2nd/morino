...
import { birth, calcSecond2Birthday } from "../assets/common.js";
...
export async function getOgImageUrl(params) {
  ...
  // TODO: なんかやばい
  const date = datetime(timestamp).toZonedTime("Asia/Tokyo");

  // 誕生日なら専用の画像を返す
  if (date.month === birth.mon && date.day === birth.day) {
    return `${Deno.env.get("BASE_URL")}/ogp-hpb.png`;
  }

  const second = calcSecond2Birthday(parseInt(date.format("x")));
